        ...
        getDefaultValues().then(async (response) => {
            for (let i = 0; i < response.length; i++) {
                if (response[i].trip_id.toString() === window.sessionStorage.getItem('curTrip')) {
                    ...
                    setSelectedStart({lat: parseFloat(res[0]), lng: parseFloat(res[1])});
                    const res2 = response[i].endLoc.split(" ")
                    setSelectedEnd({lat: parseFloat(res2[0]), lng: parseFloat(res2[1])});
                    break;
                }
            }
        })
                    ...
                    if (response[i].trip_id.toString() === window.sessionStorage.getItem('curTrip')) {
                        ...
                        response[i].route.stops.forEach((stop) => {
                            if(stop.stopName != response[i].start && stop.stopName != response[i].end) {
                                ...
                                container.classList.add(styles.listContainer);
                                container.setAttribute('role', 'listitem');
                                const listItem = document.createElement('div');
                                listItem.classList.add(styles.listItem);
                                listItem.innerHTML = stop.stopName;
                                container.onclick = removeListItem;
                                container.append(listItem);
                                t.insertBefore(container, t.lastChild);
                                setTimeout(function () {
                                    container.classList.add(styles.show);
                                    listItem.classList.add(styles.show);
                                }, 15);
                            }
                        })
                        setTrip(trip);
                    ...
                    <div className={styles.registerInput}>
                        <div className={styles.float}>
                            ...
                            <h2 className={globalStyles.gradientText}>Highways: <Checkbox label="Highways"
                                                                                          ...
                                                                                          onChange={handleHighways}
                            /></h2>
                            {selectedRoute != "" && directionsResponse != null && calculateStops() && <div className={styles.scrollable}><StopResultsNew results={stopsResponse} markers={markers} trip={trip} setTrip={setTrip}/></div>}
                        </div>
                        { !showStopPref &&
                            <div className={styles.stopFloatButton}>
                                <button onClick={()=>{setShowStopPref(!showStopPref)}}>Stop Preferences</button>
                                 <ul className={styles.list} id="tripList"><li className={`${styles.listContainer} ${styles.show}`}>
                        ...
                        <button type="button" onClick={handleSubmit}>Update Trip</button>
                    </div>
                </div>
            </div>
        </div>
